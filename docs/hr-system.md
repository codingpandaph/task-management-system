...
| Employee profile         | HR with employee permissions              | Open a person and visit Overview, Employment, Leave Policies, Access & Security                                  | Each tab shows only relevant facts and permitted actions                                                     | HRIS navigation and management flows           |
...
| Employment               | HR with employment permission             | Add a contractual record with an inclusive end date and inspect history                                          | Previous record closes; new record and history remain visible                                                | management and lifecycle flows                 |
...
| File leave               | Any eligible employee                     | Preview dates, save/edit a draft, submit it; try weekend-only, overlap, cross-year, and non-December Christmas   | Valid request snapshots working days and approvers; invalid requests show safe validation errors             | demo, matrix, date integration flows           |
...
| HR leave                 | HR with leave administration              | Add administrative leave, correct it, and post the same adjustment operation twice                               | Entry approves immediately; correction reverses atomically; retry posts once                                 | administration and idempotency flows           |
...
| Notifications/audit      | Recipient; user with audit permission     | Open notifications, follow and mark an item read; filter/read audit events                                       | Only owned notifications change; audit stays immutable and contains no credentials or restricted narratives  | management reads and privacy integration flows |
...
ledger/audit, date-boundary, replay, and privacy assertions that are impractical to demonstrate visually.
