...
| Notifications/audit      | Recipient; user with audit permission     | Filter All/Unread notifications, follow an item, mark one or all read; search audit by action or target                         | Only owned notifications change; audit stays immutable and contains no credentials or restricted narratives       | browser review, bulk read, management reads and privacy flows |
